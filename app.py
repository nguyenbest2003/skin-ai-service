...
async def ai_suggestion(payload: dict):
    ...
    try:
        ...
        print(">>> Flash lỗi → fallback Pro:", e)

    try:
        model3 = genai.GenerativeModel("models/gemini-2.0-pro")
        res3 = model3.generate_content(prompt)
        if hasattr(res3, "text") and res3.text:
            return {"goi_y_AI": res3.text}
    except Exception as e:
        print(">>> Pro lỗi → fallback 1.5 Flash:", e)

    try:
        model4 = genai.GenerativeModel("models/gemini-1.5-flash")
        res4 = model4.generate_content(prompt)
        if hasattr(res4, "text") and res4.text:
            return {"goi_y_AI": res4.text}
    except Exception as e:
        print(">>> 1.5 Flash lỗi → offline.", e)
    offline = (
