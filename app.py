...
async def ai_suggestion(payload: dict):
    ...
    if "analysis_id" in payload:
        ...
        ac = float(payload["muc_do_mun"])
        rd = float(payload["do_do"])
    ...
    prompt = (
        ...
        f"Phân tích da: mụn={ac}, đỏ={rd}, dầu={oil}, lỗ chân lông={pr}. "
    )
    if per_zone:
    ...
    # -----------------------------------------------------
   # -------------------------
# ENDPOINT: AI SUGGESTION (with correct Google models)
# -------------------------
@app.post("/ai-suggestion")
async def ai_suggestion(payload: dict):
    per_zone = payload.get("per_zone", False)

    # Lấy điểm số từ result.json hoặc payload
    if "analysis_id" in payload:
        aid = payload["analysis_id"]
        path = BASE_TMP / aid / "result.json"
        if not path.exists():
    ...
    # ============================================
    # 1) GEMINI MAIN (flash-lite-preview)
    # ============================================
    try:
        ...
        print(">>> Hết quota Flash Lite → fallback Flash Preview.")
    except Exception as e:
        print(">>> Lỗi Flash Lite → fallback:", e)

    # ============================================
    # 2) FALLBACK GEMINI FLASH (preview)
    # ============================================
    try:
        model2 = genai.GenerativeModel("models/gemini-1.5-flash-preview")
        res2 = model2.generate_content(prompt)
        if hasattr(res2, "text") and res2.text:
            return {"goi_y_AI": res2.text}
    except Exception as e:
        print(">>> Flash Preview lỗi → fallback Lite Preview:", e)

    # ============================================
    # 3) LAST FALLBACK — gemini-1.5-flash-lite-preview
    # ============================================
    try:
        model3 = genai.GenerativeModel("models/gemini-1.5-flash-lite-preview")
        res3 = model3.generate_content(prompt)
        if hasattr(res3, "text") and res3.text:
            return {"goi_y_AI": res3.text}
    except Exception as e:
        print(">>> Flash Lite Preview lỗi → offline:", e)
    ...
    offline = (
        ...
        "- Không nặn mụn.\n"
        "- Dùng BHA hoặc Niacinamide (3–4 lần/tuần).\n"
        "- Dưỡng ẩm phù hợp loại da.\n"
        "- Thoa kem chống nắng SPF 50 mỗi ngày."
    )
