...
async def ai_suggestion(payload: dict):
    ...
    if GEMINI_KEY:
        try:
            ...
            if text:
                return {"goi_y_AI": text}

        except ResourceExhausted as e:
            print(">>> Gemini hết quota → chuyển sang Gemma:", e)

        except Exception as e:
            print(">>> Gemini lỗi khác → fallback Gemma:", e)


    # -----------------------------------------------------
    # 2) GỌI GEMMA (model free, không quota)
    # -----------------------------------------------------
    try:
        model2 = genai.GenerativeModel("models/gemma-2-9b-it")
        res2 = model2.generate_content(prompt)
        text2 = getattr(res2, "text", None)
        if text2:
            return {"goi_y_AI": text2}
    except Exception as e:
        print(">>> Gemma lỗi, fallback OFFLINE:", e)

    # -----------------------------------------------------
    # 3) FALLBACK OFFLINE (không bao giờ lỗi)
    # -----------------------------------------------------
    offline = (
        "AI hiện không khả dụng — gợi ý cơ bản:\n"
        "- Rửa mặt 2 lần/ngày bằng sữa rửa mặt dịu nhẹ.\n"
        "- Không nặn mụn.\n"
        "- Dùng BHA hoặc Niacinamide (tối đa 3–4 lần/tuần).\n"
        "- Dưỡng ẩm gel cho da dầu hoặc lotion cho da khô.\n"
